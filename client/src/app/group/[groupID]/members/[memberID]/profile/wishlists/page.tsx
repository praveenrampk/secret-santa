...
export const getParamGroupIDAndMemberID = (pathName: string) => {
  const splittedPathName = pathName.split("/");

  const groupID =
    splittedPathName[
      splittedPathName.findIndex((path) => path === "group") + 1
    ];
  const memberID =
    splittedPathName[
      splittedPathName.findIndex((path) => path === "members") + 1
    ];
...
export default function WishlistsPage() {
  ...
  const getMemberData = async () => {
    try {
      const { data } = await axios.get(
        `http://localhost:3001/api/member?memberID=${memberID}`
      );
  ...
  const getMyWishLists = async () => {
    try {
      const { data } = await axios.get(
        `http://localhost:3001/api/wish-list/group/${groupID}/members/${memberID}`
      );
  ...
  return (
    <div className="bg-transparent w-full h-screen p-8 text-white flex flex-col">
      <h1 className="text-4xl font-bold mb-8">{`${name}'s`} Wishlists</h1>
      {/* Centered Input */}
